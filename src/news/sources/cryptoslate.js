export class CryptoslateSource {
  constructor() {
    this.name = 'cryptoslate';
    this.url = 'https://cryptoslate.com/feed/';
  }
  ...
  async fetchNews() {
    try {
      ...
      return items.slice(0, 5).map(item => ({
        id: `cryptoslate-${item.guid || item.link}`,
      ...
      console.error('Cryptoslate fetch error:', error);
      return [];
    }
  }
